...
func _ready():
	...
	$factoryprogress/Cylinder_001.set_surface_override_material(0,$factoryprogress/Cylinder_001.get_active_material(0).duplicate())

func frame(sincelasttick):
	#update progress bar with progress.getValue(sincelasttick)

	$factoryprogress/Cylinder_001.get_active_material(0).set_shader_parameter("progress", progress.getValue(sincelasttick)/processing_time)
...
func removeIngredients()-> bool:
	...
	for port in ports:
		if port.output==self:
			if not port.isFull()or port.content.type != port.type :
				return false
			else:
				print(port.content.type)
